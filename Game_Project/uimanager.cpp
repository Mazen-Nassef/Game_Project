...
void UIManager::resetUI() {
    score = 0;
    gameTime = 0.0;
    maxPlayerX = 0; // Reset forward tracking
    updateScore(score);
    updateTime(gameTime);
}
...
void UIManager::updateScoreByPlayerX(qreal playerX)
{
    if (playerX > maxPlayerX) {
        int distance = static_cast<int>(playerX - maxPlayerX);
        maxPlayerX = playerX;
        score += distance; // Or scale: e.g. score += distance / 10;
        updateScore(score);
        emit scoreUpdated(score);
    }
    // Else: do nothing when going backward or staying still
}
...
void UIManager::connectPlayerSignals(Player* player) {
    ...
    // Store reference for tracking movement
    trackedPlayer = player;
    lastPlayerX = static_cast<int>(player->x());
}
...
void UIManager::onGameTimerTick() {
    ...
    if (trackedPlayer) {
        int currentX = static_cast<int>(trackedPlayer->x());

        if (currentX > lastPlayerX) {
            score += 1;  // Increase score only when moving forward
            updateScore(score);
            emit scoreUpdated(score);
        }
