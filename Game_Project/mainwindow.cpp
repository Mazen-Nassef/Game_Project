...
void MainWindow::updateCamera()
{
    ...
    // Move camera based on player's position
    currentLevel->followPlayer(view);

    // Update UI label positions
    uiManager->positionLabels(view);

    // ✅ Get player's scene position and update score based on movement
    if (player) {
        qreal playerX = player->mapToScene(0, 0).x();
        uiManager->updateScoreByPlayerX(playerX);
    }
}
